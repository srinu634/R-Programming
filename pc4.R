...
excludevars <-  names(pc4) %in% c("ESSENTIAL_DENSITY")
pc4 <- pc4[ !excludevars  ]
...
pc4$c <- as.numeric(factor(pc4$c , levels=c("FALSE" ,"TRUE") ) ) 
pc4.test$c <- as.numeric(pc4.test$c , levels=c("FALSE" ,"TRUE") ) 


#discretize the data for bayesian networks
pc4.disc = chiM(pc4, alpha = 0.05)
pc4.disc <- pc4.disc[c(1:14,16:38)]
...
pc4.disc.data <- pc4.disc.data[,"ESSENTIAL_DENSITY"] 


#Building a Naive Bayes classifier
pc4.bn = naive.bayes(pc4.disc.data, "c")
pc4.pred.bn = predict(pc4.bn, pc4.disc.data)  #2nd parameter should be pc4.test.data
table(pc4.pred.bn,pc4.disc.data[,"c"])   #output the prediction matrix
