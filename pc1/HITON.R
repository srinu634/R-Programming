runHITON = function(debug) {
  ...
  pc1.si.hiton.pc.pred<- predict(pc1.si.hiton.pc.fitted$L, pc1.test.data) #2nd parameter should be pc1.test.data
  
  print( table(pc1.si.hiton.pc.pred, pc1.test.data[, "L"]) ) #output the prediction matrix
  #Change the outputs to numeric values; 
  pc1.given.si.hiton.pc <- as.numeric(as.character(pc1.si.hiton.pc.pred))
  pc1.pred.si.hiton.pc <- as.numeric(as.character(pc1.test.data[,"L"]))
