...
runClassifiers = function ( debug,i) {
  ...
  bn  <<- naive.bayes(disc.data, "L")
  pred.bn  <<- predict(bn, test.data,prob=TRUE)  #2nd parameter should be test.data
  
  
  print ( table(pred.bn,test.data[,"L"] ) )  #output the prediction matrix
  ...
  predicted.bn  <<- as.numeric(as.character(test.data[,"L"]))
  ...
  print ( table(pred.tan, test.data[, "L"]) ) #output the prediction matrix
  #Change the outputs to numeric values; 
  given.tan <<- as.numeric(as.character(pred.tan))
  pred.tan <<- as.numeric(as.character(test.data[,"L"]))
